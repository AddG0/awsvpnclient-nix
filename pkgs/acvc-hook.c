/*
 * LD_PRELOAD shim for the AWS VPN Client 5.4.0 service (see service.nix). It is
 * preloaded into the service process; the dbus-daemon and openvpn children inherit
 * it harmlessly. Two independent workarounds for two 5.4.0 behaviours that the
 * buildFHSEnv sandbox trips over:
 *
 * 1. D-Bus boolean fixup (the sendmsg/sendto/write hooks, vendored from
 *    https://github.com/BOPOHA/aws-rpm-packages awsvpnclient/hook0.c).
 *    The service's reply to IsExclusiveAppInstance encodes a malformed boolean, so
 *    the GUI throws "Read value 17 at position 4 while expecting boolean" and bails.
 *    Running inside the dbus-daemon (which forwards both the call and the reply) the
 *    hook rewrites the reply's boolean to true.
 *
 * 2. Caller binary-path fixup (the readlink/readlinkat hooks).
 *    The service guards every D-Bus method with ValidatePidBinaryPath, which
 *    readlink()s /proc/<caller>/exe and requires the directory to be exactly
 *    /opt/awsvpnclient. In the sandbox the GUI's exe resolves to its /nix/store
 *    path, so the check throws. We rewrite a /proc/<pid>/exe target that ends in
 *    /opt/awsvpnclient/awsvpnclient (the GUI) back to that bare path. acvc-openvpn
 *    and everything else are left untouched, so the openvpn path check still works.
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
/*
 * readlink / readlinkat fixup for ValidatePidBinaryPath (see file header, point 2).
 * The service readlink()s /proc/<pid>/exe; when the caller is the sandboxed GUI the
 * target is /nix/store/.../opt/awsvpnclient/awsvpnclient. Rewrite that to the bare
 * /opt/awsvpnclient/awsvpnclient so the directory check passes. Anything else
 * (notably acvc-openvpn) is returned unchanged.
 */
#define GUI_EXE_PATH "/opt/awsvpnclient/awsvpnclient"
...
/* readlink() does not NUL-terminate; n is the byte count written to buf. */
static ssize_t rewrite_gui_exe(const char *pathname, char *buf, ssize_t n, size_t bufsiz) {
    if (n <= 0 || !is_proc_exe(pathname)) return n;

    char target[4096];
    size_t copy = (size_t)n < sizeof(target) - 1 ? (size_t)n : sizeof(target) - 1;
    memcpy(target, buf, copy);
    target[copy] = '\0';

    /* The first "/opt/awsvpnclient/" splits off the store prefix; the remainder
       must be exactly the GUI executable for us to rewrite it. */
    const char *suffix = strstr(target, GUI_EXE_PATH);
    if (!suffix || strcmp(suffix, GUI_EXE_PATH) != 0) return n;

    size_t out_len = strlen(GUI_EXE_PATH);
    if (out_len > bufsiz) out_len = bufsiz;
    memcpy(buf, GUI_EXE_PATH, out_len);
    fprintf(stderr, "[HOOK] Rewrote %s target to %s\n", pathname, GUI_EXE_PATH);
...
ssize_t readlink(const char *pathname, char *buf, size_t bufsiz) {
    ...
    return rewrite_gui_exe(pathname, buf, orig(pathname, buf, bufsiz), bufsiz);
}
...
ssize_t readlinkat(int dirfd, const char *pathname, char *buf, size_t bufsiz) {
    ...
    return rewrite_gui_exe(pathname, buf, orig(dirfd, pathname, buf, bufsiz), bufsiz);
}
